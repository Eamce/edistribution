      ...
      child: WillPopScope(
        ...
        child: Scaffold(
          ...
          appBar: AppBar(
            ...
            titleSpacing: 0.0,
            leading: GlobalVariables.logcustomerFirstLogDone
                ? IconButton(
                    icon: Icon(Icons.arrow_back, color: Colors.black),
                    onPressed: () => Navigator.of(context).pop(),
                  )
                : IconButton(
                    icon: Icon(CupertinoIcons.lock_shield, color: Colors.black),
                    onPressed: () {},
                  ),
  ...
  saveChanges() async {
    ...
    if (con == 'OKAY') {
      var res = await changePass(
          context, GlobalVariables.logcustomerCode, newPassController.text);
      if (res != "passwordused" && res != null) {
        connectionChecking = false;
        connectionChecingStatus = '';
        if (mounted) setState(() {});
        customLogicalModal(
            context, Text('Password successfully changed. Logout?'), () {
          //no logout
        ...
        customModal(
            ...
            Text("You already used this password. Try a different one.",
                textAlign: TextAlign.center),
            true,
            Icon(
              CupertinoIcons.checkmark_alt,
